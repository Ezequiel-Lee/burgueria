        ...
        cartItemElement.innerHTML = `
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="font-medium my-2">R$ ${item.price.toFixed(2)}</p>
                </div>

                <button class="remove-cart-from-btn bg-red-500 text-white px-4 py-1 rounded" data-name="${item.name}">
                    Remover
                </button>
            </div>
        `
...
checkoutBtn.addEventListener("click", function () {
    ...
    if (!isOpen) {
        Toastify({
            ...
            stopOnFocus: true,
            style: {
              background: "#e50914", borderRadius: "5px",
            },
        }).showToast();
        return;
    }
    ...
    if (addressInput.value === "") {       
        ...
        addressWarn.classList.add("flex")
